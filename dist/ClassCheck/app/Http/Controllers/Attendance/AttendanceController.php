...
class AttendanceController extends Controller
{
    ...
    public function show(Request $request, AttendanceSession $attendanceSession): Response
    {
        ...
        $seats = Seat::query()
            ...
            ->map(function ($seat) use ($recordsByStudent, $absentCounts) {
                ...
                return [
                    ...
                    'student' => $seat->student ? $this->studentData($seat->student, $absentCounts) : null,
        ...
        $unseated = $attendanceSession->records
            ...
            ->sortBy([
                fn ($a, $b) => strcasecmp($a->student?->last_name ?? '', $b->student?->last_name ?? '') ?: strcasecmp($a->student?->first_name ?? '', $b->student?->first_name ?? ''),
            ])
            ->map(fn ($record) => [
                'student' => $this->studentData($record->student, $absentCounts),
    ...
    private function studentData($student, array $absentCounts = []): array
    {
        $firstMiddle = collect([$student->first_name, $student->middle_name])->filter()->join(' ');
        $formattedName = $student->last_name && $firstMiddle
