...
const qrDataUrl = ref('');
onMounted(async () => {
    qrDataUrl.value = await QRCode.toDataURL(props.join_url, { width: 640, margin: 2, color: { dark: '#1c1917', light: '#fffdf7' } });
});
const printQr = () => {
...
<template>
    ...
    >
        <main class="min-h-full bg-[#f5f0e6] p-4 md:p-8">
            <div class="mx-auto max-w-7xl">
                <div
                    v-if="page.props.flash?.success"
                    class="mb-5 rounded-xl border border-emerald-300 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-900"
                >
                    {{ page.props.flash.success }}
                </div>
                <header class="relative overflow-hidden rounded-3xl bg-stone-950 p-6 text-stone-50 shadow-2xl md:p-8">
                    <div
                    ...
                    <div class="relative flex flex-col justify-between gap-7 lg:flex-row lg:items-end">
                        <div>
                            <p class="font-mono text-xs font-bold uppercase tracking-[.28em] text-amber-400">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            ><Button class="bg-amber-500 text-stone-950 hover:bg-amber-400" @click="showQr = true"
                ...
                <div class="mt-6 grid gap-6 xl:grid-cols-[1fr_330px]">
                    <section class="min-w-0 rounded-3xl border border-stone-300 bg-[#fffdf7] p-5 shadow-[0_25px_80px_-60px_rgba(28,25,23,.8)] md:p-7">
                        <div class="mb-7 flex flex-wrap items-center justify-between gap-4">
                            <div>
                                <p class="font-mono text-xs font-bold uppercase tracking-[.2em] text-amber-700">Live seating chart</p>
                            ...
                            <div class="flex items-center gap-4 text-xs text-stone-500">
                                <span class="flex items-center gap-1"><i class="size-3 rounded-sm bg-emerald-500" /> Assigned</span
                                ><span class="flex items-center gap-1"><i class="size-3 rounded-sm border border-stone-400 bg-white" /> Free</span>
                            </div>
                        </div>
                        <div
                            class="mb-8 rounded-xl border-2 border-stone-800 bg-stone-800 py-3 text-center font-mono text-xs font-bold uppercase tracking-[.35em] text-stone-100"
                        >
                            Teaching wall / board
                        </div>
                        <div
                        ...
                        <div v-else class="rounded-2xl border border-dashed border-stone-300 p-10 text-center">
                            <h3 class="mt-3 font-serif text-2xl font-bold">Set the room rows and columns</h3>
                            <p class="mt-2 text-sm text-stone-500">Use Room setup to create the chair grid, then drag aisles into the preview.</p>
                        </div>
                    </section>
                    ...
                    <aside class="space-y-5">
                        ...
                        <form class="rounded-2xl border border-stone-300 bg-[#fffdf7] p-5" @submit.prevent="addStudent">
                            <p class="text-xs font-bold uppercase tracking-wider text-amber-700">Manual roster entry</p>
                            <div
                                class="mt-3 rounded-xl border px-3 py-2.5 text-sm"
                                :class="
                                    selectedSeat ? 'border-amber-400 bg-amber-100 text-stone-900' : 'border-stone-200 bg-stone-100 text-stone-600'
                                "
                            >
                                <div class="flex items-center justify-between gap-3">
                                    ...
                                    <button
                                        v-if="selectedSeat"
                                        type="button"
                                        class="shrink-0 text-xs font-bold text-amber-900 underline"
                            ...
                            <p class="mt-2 text-xs text-stone-500">Click any empty chair on the classroom floor to select it.</p>
                            <div class="mt-4 grid gap-3">
                                ...
                                <div>
                                    <Label for="student-middle-name">Middle name <span class="text-stone-400">(optional)</span></Label>
                                ...
                                <div>
                                    <Label for="student-photo" class="mb-1 block">Photo <span class="text-stone-400">(optional)</span></Label>
                                    <input
                                        id="student-photo"
                                        type="file"
                            ...
                            <Button type="submit" class="mt-4 w-full bg-amber-700 text-white" :disabled="studentForm.processing">
                                <UserPlus class="mr-2 size-4" /> {{ studentForm.processing ? 'Adding student...' : 'Add student' }}
                            </Button>
                        </form>
                        <div class="rounded-2xl border border-stone-300 bg-[#fffdf7] p-5">
                            <div class="flex items-center justify-between">
                                <span class="flex items-center gap-2 font-semibold"><Users class="size-4 text-amber-700" /> Roster</span
                                ><strong class="font-mono text-xl">{{ section.students.length }}</strong>
                            </div>
                            <form
                                class="mt-4 rounded-xl border border-dashed border-stone-300 p-3"
                            ...
                            <div class="mt-4 max-h-80 overflow-auto">
                                <div class="space-y-1">
                                    <button
                                        ...
                                        :key="student.id"
                                        type="button"
                                        class="flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-stone-100"
                                        @click="selectedStudent = student"
                                    >
                                        <span class="truncate">{{ student.full_name }}</span>
                                        <span class="text-[10px] text-stone-400">{{ student.seat.label }}</span>
                                    </button>
                                    <p v-if="!seatedStudents.length" class="px-3 py-2 text-xs text-stone-500">No students have a chair yet.</p>
                                </div>
                                <div v-if="unseatedStudents.length" class="mt-4 border-t border-dashed border-stone-300 pt-3">
                                    <p class="mb-1 px-3 text-xs font-bold uppercase tracking-wider text-stone-500">Without chair</p>
                                    <button
                                        v-for="student in unseatedStudents"
                                        :key="student.id"
                                        type="button"
                                        class="block w-full truncate rounded-lg px-3 py-2 text-left text-sm hover:bg-stone-100"
                                        @click="selectedStudent = student"
                                    >
                                        {{ student.full_name }}
                                    </button>
            ...
            <div v-if="showQr" class="fixed inset-0 z-50 grid place-items-center bg-stone-950/70 p-4 backdrop-blur-sm" @click.self="showQr = false">
                <section class="w-full max-w-md rounded-3xl bg-[#fffdf7] p-6 text-center shadow-2xl">
                    <button class="ml-auto block rounded-full p-2 hover:bg-stone-100" @click="showQr = false"><X class="size-5" /></button>
                    <p class="font-mono text-xs font-bold uppercase tracking-[.2em] text-amber-700">Student enrollment</p>
                    ...
                    <p class="break-all rounded-lg bg-stone-100 p-3 font-mono text-[10px]">{{ join_url }}</p>
                    ...
                    <div class="mt-2 flex justify-center gap-2">
                        <a
                            ...
                            class="rounded-md px-3 py-2 text-xs font-semibold hover:bg-stone-100"
                            >Download PNG</a
                        ><button type="button" class="rounded-md px-3 py-2 text-xs font-semibold hover:bg-stone-100" @click="printQr">
                            Print QR
                        </button>
                    </div>
                    ...
                    >
                    <p class="mt-2 text-xs font-semibold" :class="section.enrollment_open ? 'text-emerald-700' : 'text-red-700'">
                        Enrollment is {{ section.enrollment_open ? 'OPEN' : 'CLOSED' }}
                    </p>
                </section>
            </div>

            <div v-if="selectedStudent" class="fixed inset-0 z-40 bg-stone-950/40" @click.self="selectedStudent = null">
                <aside class="ml-auto flex h-full w-full max-w-md flex-col bg-[#fffdf7] p-6 shadow-2xl">
                    <button class="ml-auto rounded-full p-2 hover:bg-stone-100" @click="selectedStudent = null"><X class="size-5" /></button>
                    <div class="mt-4 flex items-center gap-4">
                        <img v-if="selectedStudent.photo_url" :src="selectedStudent.photo_url" alt="" class="size-20 rounded-2xl object-cover" />
                        <div v-else class="grid size-20 place-items-center rounded-2xl bg-amber-100 font-serif text-3xl font-bold text-amber-800">
                            {{ selectedStudent.first_name[0] }}{{ selectedStudent.last_name[0] }}
                        </div>
                        <div>
                            <h2 class="font-serif text-2xl font-bold">{{ selectedStudent.full_name }}</h2>
                            <p class="font-mono text-xs text-stone-500">{{ selectedStudent.student_number }}</p>
                        </div>
                    </div>
                    <dl class="mt-8 grid grid-cols-2 gap-4 border-y border-dashed border-stone-300 py-5">
                        <div>
                            <dt class="text-xs text-stone-500">Current chair</dt>
                            <dd class="mt-1 font-semibold">{{ selectedStudent.seat?.label || 'Unseated' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs text-stone-500">Section</dt>
                    ...
                    <div class="mt-6">
                        ...
                        ><select
                            class="mt-2 w-full rounded-md border-stone-300"
