    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <SettingsLayout>
            <div class="flex flex-col space-y-8">
                ...
                <div
                    v-if="flashSuccess"
                    class="flex items-start gap-3 rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-4 text-emerald-900 dark:text-emerald-200"
                >
                ...
                <div
                    v-if="flashError || restoreForm.errors.backup_file"
                    class="flex items-start gap-3 rounded-lg border border-red-500/30 bg-red-500/10 p-4 text-red-900 dark:text-red-200"
                >
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <div class="flex flex-col rounded-xl border border-border bg-card p-4 shadow-sm">
                        ...
                        <div class="mt-2 text-lg font-bold capitalize text-foreground">
                            {{ driver }} {{ isSqlite ? '(Offline / Standalone)' : '(Cloud / Online)' }}
                        </div>
                        <div v-if="sqliteSize" class="mt-1 text-xs text-muted-foreground">File Size: {{ formatBytes(sqliteSize) }}</div>
                    </div>
                    ...
                    <div class="flex flex-col rounded-xl border border-border bg-card p-4 shadow-sm">
                        ...
                        <div class="mt-2 text-lg font-bold text-foreground">{{ stats.sections_count }} Active Sections</div>
                        <div class="mt-1 text-xs text-muted-foreground">Across {{ stats.terms_count }} Academic Terms</div>
                    </div>
                    ...
                    <div class="flex flex-col rounded-xl border border-border bg-card p-4 shadow-sm">
                        ...
                        <div class="mt-2 text-lg font-bold text-foreground">{{ stats.students_count }} Students</div>
                        <div class="mt-1 text-xs text-muted-foreground">With seating, attendance & grades</div>
                    </div>
                </div>
                ...
                <div class="rounded-xl border border-border bg-card p-6 shadow-sm">
                    <div class="flex items-center justify-between border-b border-border pb-4">
                        ...
                        <a :href="route('backup.export-json')" class="inline-flex">
                            <Button class="gap-2 bg-emerald-600 text-white shadow-sm hover:bg-emerald-700">
                ...
                <div v-if="isSqlite" class="rounded-xl border border-border bg-card p-6 shadow-sm">
                    <div class="flex items-center justify-between border-b border-border pb-4">
                        ...
                        <div class="flex items-center gap-2">
                            <Button variant="outline" class="gap-2" :disabled="isCreatingSnapshot" @click="createSnapshot">
                                <RefreshCw :class="['h-4 w-4', { 'animate-spin': isCreatingSnapshot }]" />
                                Save Local Snapshot
                            </Button>

                            <a :href="route('backup.download-sqlite')" class="inline-flex">
                                <Button class="gap-2 bg-blue-600 text-white shadow-sm hover:bg-blue-700">
                                    <Download class="h-4 w-4" />
                                    Download SQLite DB
                                </Button>
                            </a>
                    ...
                    <div v-if="localSnapshots.length > 0" class="mt-4">
                        ...
                        <div class="divide-y divide-border rounded-lg border border-border bg-muted/30">
                            <div v-for="snap in localSnapshots" :key="snap.name" class="flex items-center justify-between px-4 py-2.5 text-xs">
                ...
                <div class="rounded-xl border border-border bg-muted/20 p-5">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="space-y-1.5 text-xs text-muted-foreground">
                            <div class="text-sm font-semibold text-foreground">How to Sync Data Between Offline & Online</div>
                            <p>
                                <strong>From Offline PC to Online Web:</strong> Click <em>Export JSON Backup</em> on your offline laptop, then open
                                the online school portal, navigate to <em>Settings → Backup & Export</em>, select the file and click
                                <em>Restore Backup</em>.
                            </p>
                            <p>
                                <strong>From Online Web to Offline PC:</strong> Download the JSON backup from the web portal and restore it inside
                                your offline ClassCheck app. All your seating plans, attendance records, and gradebooks will be synced seamlessly.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div v-if="showConfirmModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                <div class="w-full max-w-md rounded-xl border border-border bg-card p-6 shadow-xl">
                    ...
                    <p class="mt-2 text-sm text-muted-foreground">
                        ...
                        <code class="font-mono text-xs font-semibold text-foreground">{{ restoreForm.backup_file?.name }}</code
                        >?
                    </p>
                    <div
                        v-if="restoreForm.clean_replace"
                        class="mt-3 rounded-md border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-900 dark:text-amber-200"
                    >
                        <strong>Warning:</strong> Clean Replace Mode is selected. Existing sections for this user account will be overwritten.
                    </div>
                    <div
                        v-else
                        class="mt-3 rounded-md border border-emerald-500/30 bg-emerald-500/10 p-3 text-xs text-emerald-900 dark:text-emerald-200"
                    >
                    ...
                    <div class="mt-6 flex justify-end gap-3">
                        ...
                        <Button class="bg-emerald-600 text-white hover:bg-emerald-700" @click="confirmRestore"> Confirm & Restore </Button>
                    </div>
                </div>
            </div>
        </SettingsLayout>
